...
type Highlight = { i: "HEART" | "BROWSER" | "PLANT"; t: string; c: string };
          ...
          <form className={"contact-form" + (shake ? " shake" : "")} onSubmit={onSubmit}>
            {status !== "success" ? (
              <>
                ...
                {status === "error" ? (
                  <button className="btn ghost" type="button" style={{ width: "100%", marginTop: 10 }} onClick={onRetry}>
                    REINTENTAR
                  </button>
                ) : null}
              </>
...
function HighlightIcon({ kind }: { kind: Highlight["i"] }) {
