// ===== app-data.ts — datos ficticios (placeholder de una futura base de datos) =====
...
export function getSeededScores(seed: number, count = 12): ScoreRow[] {
  let s = seed;
  ...
  for (let i = 0; i < count; i++) {
    let name: string;
    do {
      name = PLAYER_NAMES[Math.floor(rand() * PLAYER_NAMES.length)];
    } while (used.has(name) && used.size < PLAYER_NAMES.length);
    used.add(name);
