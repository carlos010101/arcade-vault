...
- `middleware.ts` en la raíz para refrescar la sesión de Supabase en cada request (patrón estándar de `@supabase/ssr` para App Router), aunque todavía no exista ningún flujo de auth conectado.
- Variables de entorno `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY`, obtenidas del proyecto Supabase ya enlazado (`crkexepgfehipluoeyqc`, ver `.mcp.json`) vía las tools de Supabase MCP (`get_project_url`, `get_publishable_keys`). Se agregan con valores reales a `.env.local` (gitignoreado) y como claves sin valor a `.env.example`.
...
2. Obtener `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` vía `mcp__supabase__get_project_url` y `mcp__supabase__get_publishable_keys` sobre el proyecto `crkexepgfehipluoeyqc`; agregarlas a `.env.local` con sus valores reales y a `.env.example` sin valor (junto a `RESEND_API_KEY` y `SUPABASE_DB_PASSWORD` ya existentes).
3. Crear `lib/supabase/client.ts`: exporta una función `createClient()` que llama a `createBrowserClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!)`.
...
5. Crear `middleware.ts` en la raíz del proyecto: usa un cliente Supabase de middleware (`createServerClient` con `NextRequest`/`NextResponse`) para refrescar el token de sesión en cada request, siguiendo el patrón oficial de `@supabase/ssr`; `matcher` que excluye assets estáticos.
...
- [ ] `lib/supabase/client.ts` y `lib/supabase/server.ts` existen y exportan clientes tipados sin errores de TypeScript (`npm run build` compila).
- [ ] `middleware.ts` existe y refresca la sesión sin lanzar errores al navegar cualquier ruta existente (`/`, `/biblioteca`, `/auth`, `/salon`, `/juego/[id]`).
- [ ] `.env.local` contiene `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` con valores reales; `.env.example` las documenta sin valor.
- [ ] `GET /api/health-db` devuelve `200 { ok: true }` en local.
- [ ] `mcp__supabase__list_tables` sigue devolviendo `[]` (no se crearon tablas de negocio).
- [ ] Ninguna pantalla existente (`app/auth/page.tsx`, `lib/session-context.tsx`, `lib/app-data.ts`) fue modificada.
- [ ] `npm run lint` pasa sin nuevos errores.
...
- **Variables de entorno mal alcanzadas (`NEXT_PUBLIC_` vs. server-only):** usar por error `NEXT_PUBLIC_` en una clave que no debería exponerse al cliente filtraría secretos al bundle. Mitigación: solo `URL` y `ANON_KEY` (públicas por diseño en Supabase) llevan el prefijo `NEXT_PUBLIC_`; cualquier clave `service_role` queda fuera de alcance de esta spec.
- **Middleware mal configurado puede bloquear rutas existentes:** un `matcher` incorrecto en `middleware.ts` podría interceptar rutas estáticas o romper el Nav. Mitigación: seguir el patrón oficial de `@supabase/ssr` con `matcher` que excluye `_next/static`, `_next/image`, `favicon.ico` y assets.
